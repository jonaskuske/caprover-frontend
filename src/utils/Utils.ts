...
export function formatToRelativeTime(date: Date) {
	...
	if (d > 3) return dateFormatter.format(date)
	if (h > 23) return relativeTimeFormatter.format(-Math.floor(d), 'days')
	if (m > 59) return relativeTimeFormatter.format(-Math.floor(h), 'hours')
	if (s > 59) return relativeTimeFormatter.format(-Math.floor(m), 'minutes')
	return relativeTimeFormatter.format(-Math.floor(s), 'seconds')
}
